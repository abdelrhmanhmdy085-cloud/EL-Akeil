...
function initLanguage() {
    ...
    if (nav) {
        // Remove obsolete select if it exists
        const oldSelect = nav.querySelector('select.lang-selector');
        if (oldSelect) {
            oldSelect.remove();
        }

        // Prevent duplicates and prepend semantic button
        if (!document.getElementById('lang-toggle-btn')) {
            ...
            const currentLabel = saved === 'ar' ? 'EN' : 'AR';
            toggleBtn.innerHTML = `🌍 <span id="lang-label">${currentLabel}</span>`;
