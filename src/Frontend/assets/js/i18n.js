...
function initLanguage() {
    ...
    if (nav) {
        nav.innerHTML = `
            <button type="button" class="lang-toggle-btn" onclick="toggleLanguage()" data-i18n-title="lang_toggle_title" data-i18n-aria-label="aria_label_lang_toggle">
                🌍 <span id="lang-label">${saved === 'ar' ? 'EN' : 'AR'}</span>
            </button>
        `;
    }
    
    // Fix: Ensure translations are applied AFTER elements are injected
